...
// ─── Activity Block ───────────────────────────────────────────────────────────
function ActivityBlock({ activity, onPress, delay = 0 }:
  ...
  return (
    <Animated.View style={[s.actWrap, {
      top, height,
      opacity: anim,
      transform: [{ translateX: anim.interpolate({ inputRange:[0,1], outputRange:[-10,0] }) }],
    }]}>
      <TouchableOpacity onPress={() => onPress(activity)} activeOpacity={0.75}
...
// ─── Now Line ─────────────────────────────────────────────────────────────────
function NowLine({ accent }: { accent: string }) {
  const y = getNowMins() * PX_PER_MIN;
  return (
    <View style={[s.nowLine, { top: y, backgroundColor: accent }]} pointerEvents="none">
      <View style={[s.nowDot, { backgroundColor: accent }]} />
    </View>
  );
}

// ─── Detail Drawer ────────────────────────────────────────────────────────────
function ActivityDrawer({ activity, visible, onClose, onEdit, onDelete, accent }:
  { activity: Activity|null; visible: boolean; onClose: ()=>void;
    onEdit: ()=>void; onDelete: ()=>void; accent: string }) {
    ...
    <>
      ...
      <Animated.View style={[s.drawer, { transform: [{ translateY: slideAnim }] }]} pointerEvents={visible?'auto':'none'}>
        ...
        <View style={s.metaRow}>
          {[`⏰ ${activity.startTime}`, `⏱ ${formatDuration(activity.duration)}`, `→ ${endTime}`,
            ...(activity.notifyBefore&&activity.notifyBefore>0 ? [`🔔 ${activity.notifyBefore}m`] : [])
          ].map((l) => (
        ...
        <View style={s.drawerActions}>
          <TouchableOpacity onPress={onEdit}
            style={[s.drawerBtn, { backgroundColor: accent+'12', borderColor: accent+'40' }]}
            activeOpacity={0.8}
          >
            <Text style={[s.drawerBtnTxt, { color: accent }]}>✏️  Editar</Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={onDelete}
            style={[s.drawerBtn, { backgroundColor:'rgba(255,106,142,.08)', borderColor:'rgba(255,106,142,.25)' }]}
            activeOpacity={0.8}
          >
            <Text style={[s.drawerBtnTxt, { color:'#ff6a8e' }]}>🗑  Eliminar</Text>
          </TouchableOpacity>
        </View>
      </Animated.View>
    </>
...
export default function ScheduleScreen() {
  ...
  const handleActivityPress = useCallback((a: Activity) => { setSelected(a); setDrawerOpen(true); }, []);

  const handleEdit = useCallback(() => {
    if (!selected || !activeTemplate) return;
    setDrawerOpen(false);
    setTimeout(() => {
      router.push({ pathname:'/activity/[id]', params:{ id:selected.id, templateId:activeTemplate.id, from:'schedule' } });
    }, 250);
  }, [selected, activeTemplate]);

  const handleDeleteActivity = useCallback(() => {
    if (!selected || !activeTemplate) return;
    Alert.alert('Eliminar actividad', `¿Eliminar "${selected.name}"?`, [
  ...
  const handleNew = useCallback(() => {
    ...
    router.push({ pathname:'/activity/[id]', params:{ id:'new', templateId:activeTemplate.id, from:'schedule' } });
  }, [activeTemplate]);
    ...
    <SafeAreaView style={s.container}>
      <View style={s.header}>
        ...
        <TouchableOpacity onPress={handleNew}
          style={[s.addBtn, { backgroundColor: accent }]} activeOpacity={0.8}>
      ...
      >
        {templates.map((t, i) => (
          <TouchableOpacity key={t.id}
            ...
            delayLongPress={400}
            style={[s.tab, activeId===t.id && { backgroundColor: accent+'12', borderColor: accent }]}
            activeOpacity={0.7}
          >
            <View style={[s.tabDot, { backgroundColor: i===0 ? accent : '#ff6a8e' }]} />
            <Text style={[s.tabText, activeId===t.id && { color: accent }]}>{t.name}</Text>
          </TouchableOpacity>
        ))}
        <TouchableOpacity
      ...
      >
        ...
        <NowLine accent={accent} />
      </ScrollView>

      <ActivityDrawer
        activity={selected} visible={drawerOpen}
        onClose={() => setDrawerOpen(false)}
        onEdit={handleEdit} onDelete={handleDeleteActivity}
        accent={accent}
      />
    </SafeAreaView>
  );
...
const s = StyleSheet.create({
  ...
  nowLine:        { position:'absolute', left:LABEL_W, right:0, height:1 },
  nowDot:         { position:'absolute', left:-4, top:-4, width:9, height:9, borderRadius:4.5 },
  ...
  drawerBtnTxt:   { fontSize:14, fontWeight:'500' },
});
