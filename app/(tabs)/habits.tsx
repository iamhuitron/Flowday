...
function useHabitStats(habitId: string, days = 30) {
  ...
  return Array.from({ length: days }, (_, i) => {
    ...
    return {
      date: dKey(d),
      done: habitLogs.find((l) => l.habitId === habitId && l.date === dKey(d))?.done ?? false,
    };
  });
}
...
// ─── Month Heatmap (all habits combined) ─────────────────────────────────────
function MonthHeatmap() {
  ...
  function cellColor(pct: number, isFuture: boolean, isToday: boolean) {
    ...
    if (isToday)   return pct > 0 ? `rgba(124,106,255,${0.25 + pct * 0.75})` : 'rgba(124,106,255,.18)';
      ...
      <View style={s.heatmapGrid}>
        {days.map(({ dk, pct, isFuture, isToday }) => (
          <View
            ...
            style={[
              s.heatCell,
              { backgroundColor: cellColor(pct, isFuture, isToday) },
...
// ─── Per-habit 30-day bar chart (bottom drawer) ───────────────────────────────
function HabitStatsDrawer({
  habit, visible, onClose, onEdit,
}: {
  habit: Habit | null; visible: boolean;
  ...
  // FIX New Architecture: animaciones separadas, no parallel
  const show = () => {
  ...
  useRef(visible); // keep ref in sync
    ...
    <>
      ...
      <Animated.View style={[s.statsDrawer, { transform: [{ translateY: slideAnim }] }]} pointerEvents={visible ? 'auto' : 'none'}>
        ...
        <View style={s.statsKpis}>
          {[
            { label: '28 DÍAS',   value: `${rate}%`,       color: color },
          ...
          {data.map(({ date, done }) => (
            <View key={date} style={s.barCol}>
              <View style={[s.barTrack]}>
                <View style={[
                  s.barFill,
                  { height: done ? maxBar : 4, backgroundColor: done ? color : '#1c1c22' },
                ]} />
              </View>
            </View>
          ))}
        ...
        {days.map((d, i) => {
          ...
          return (
            <View key={i} style={s.wdayCol}>
              ...
              <View style={[s.wdayCircle, isToday && s.wdayToday, allDone && s.wdayAllDone, !allDone && hasDone && s.wdayHasDone]}>
                <Text style={[s.wdayNum, isToday && { color: '#7c6aff' }, (allDone || hasDone) && !isToday && { color: '#4ade80' }]}>
                  {d.getDate()}
                </Text>
              </View>
            </View>
          ...
          return (
            <View key={i} style={s.barWrap}>
              <View style={s.barTrackWeek}>
                <View style={[s.barFillWeek, { height: `${Math.round(pct * 100)}%` as any, backgroundColor: dk === td ? '#7c6aff' : pct > 0 ? 'rgba(124,106,255,.4)' : '#1c1c22' }]} />
              </View>
            </View>
          );
        })}
  ...
  const handleLongPress = useCallback((habit: Habit) => {
    Alert.alert(habit.name, '', [
      { text: '✏️  Editar', onPress: () => router.push({ pathname: '/habit/[id]', params: { id: habit.id } }) },
      {
        ...
        onPress: () =>
          Alert.alert('Eliminar hábito', `¿Eliminar "${habit.name}" y su historial?`, [
    ...
    <SafeAreaView style={s.container}>
      ...
      <View style={s.viewToggle}>
        {(['week','month'] as const).map((v) => (
          ...
          >
            <Text style={[s.viewBtnTxt, view === v && s.viewBtnTxtActive]}>
      ...
      {/* Chart area */}
      {view === 'week' ? <WeekStrip /> : <MonthHeatmap />}
      ...
      <ScrollView style={s.scroll} contentContainerStyle={{ paddingBottom: 100 }} showsVerticalScrollIndicator={false}>
        ...
        <TouchableOpacity
          ...
          onPress={() => router.push({ pathname: '/habit/[id]', params: { id: 'new' } })}
        >
      ...
      <HabitStatsDrawer
        ...
        onEdit={() => {
          setDrawerOpen(false);
          if (selectedHabit) router.push({ pathname: '/habit/[id]', params: { id: selectedHabit.id } });
        }}
      />
    </SafeAreaView>
  );
...
const s = StyleSheet.create({
  ...
  viewBtnActive:  { backgroundColor: 'rgba(124,106,255,.12)', borderColor: '#7c6aff' },
  viewBtnTxt:     { fontFamily: MONO, fontSize: 11, color: '#55556a' },
  viewBtnTxtActive:{ color: '#7c6aff' },
  // Week strip
  ...
  heatCellToday:  { borderWidth: 1, borderColor: '#7c6aff' },
