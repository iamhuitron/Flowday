...
const PRIORITY_COLORS  = { low: '#4ade80', mid: '#fbbf24', high: '#f87171' } as const;
const PRIORITY_LABELS  = { low: 'Baja',    mid: 'Media',   high: 'Alta'   } as const;
...
// ─── Now Card ─────────────────────────────────────────────────────────────────
function NowCard({ activity, next, accent }: { activity:Activity; next?:Activity; accent:string }) {
  const color   = activity.color ?? CATEGORY_COLORS[activity.category];
  const icon    = CATEGORY_ICONS[activity.category];
  const nowMins = getNowMins();
  const startM  = timeToMins(activity.startTime);
  const pct     = Math.min(Math.round(((nowMins-startM)/activity.duration)*100), 100);
  ...
  return (
    <View style={s.sectionWrap}>
      <View style={[s.nowCard,{backgroundColor:color+'0d',borderColor:color+'33'}]}>
        ...
        {activity.description ? <Text style={s.nowDesc} numberOfLines={2}>{activity.description}</Text> : null}
...
function TaskItem({ task, onToggle, onDelete }: { task:Task; onToggle:()=>void; onDelete:()=>void }) {
  ...
  return (
    <Animated.View style={{transform:[{scale:scaleAnim}]}}>
      <TouchableOpacity onPress={handleToggle} onLongPress={onDelete} delayLongPress={400}
        activeOpacity={0.75} style={[s.taskRow,task.done&&s.taskRowDone]}
      >
        ...
        {task.priority && (
          <View style={[s.priorityBadge,{backgroundColor:PRIORITY_COLORS[task.priority]+'22',borderColor:PRIORITY_COLORS[task.priority]+'66'}]}>
...
function HabitItem({ habit, done, streak, onToggle }: {
  habit:{id:string;name:string;icon:string}; done:boolean; streak:number; onToggle:()=>void;
}) {
  ...
  const toggle = () => {
    onToggle();
    Animated.spring(scaleAnim,   {toValue:0.96,useNativeDriver:true,tension:200,friction:8}).start(()=>
  ...
  return (
    <TouchableOpacity onPress={toggle} activeOpacity={0.75} style={{flex:1}}>
      <Animated.View style={[s.habitCard,{borderColor,backgroundColor:bgColor,transform:[{scale:scaleAnim}]}]}>
  ...
  return (
    <View>
      {items.map((a)=>{
        ...
        const isPast=start+a.duration<=now, isCurrent=start<=now&&now<start+a.duration;
...
export default function TodayScreen() {
  const {
    habits, habitLogs, toggleHabitLog, getStreakForHabit, streak,
    getTodayTemplate, tasks, addTask, toggleTask, deleteTask,
  ...
  const current = template?.activities.find((a)=>{
    const sm=timeToMins(a.startTime); return nowMins>=sm&&nowMins<sm+a.duration;
  });
  ...
  const handleAddTask = useCallback(()=>{
    ...
    addTask({ id:`t-${uid()}`, name:taskText.trim(), done:false, priority:taskPriority, createdAt:new Date().toISOString() });
    setTaskText('');
    setTaskPriority(undefined);
  },[taskText,taskPriority,addTask]);
  ...
  const handleMood = useCallback((m:string)=>{
    ...
    addJournalEntry({ id:`mood-${today}`, date:today, text:'', mood:moodNum, tags:['mood'] });
  ...
  const dayNames: Record<string,string> = {mon:'Lun',tue:'Mar',wed:'Mié',thu:'Jue',fri:'Vie',sat:'Sáb',sun:'Dom'};
  ...
  return (
    <SafeAreaView style={s.container}>
      ...
      <View style={s.header}>
        ...
        <View style={[s.streakBadge,{borderColor:accent+'40',backgroundColor:accent+'12'}]}>
          <Text style={[s.streakNum,{color:accent}]}>{streak}</Text>
          <Text style={[s.streakLbl,{color:accent+'aa'}]}>días 🔥</Text>
        </View>
      </View>
      ...
      <ScrollView style={s.scroll} contentContainerStyle={{paddingBottom:100}} showsVerticalScrollIndicator={false}>
        ...
        <View style={s.statsRow}>
          ...
          <StatCard label="RUTA"    value={`${globalPct}%`}                  color={accent}  sub="global"/>
        </View>
        ...
        {current ? (
          <>
            ...
            <NowCard activity={current} next={next} accent={accent}/>
          </>
        ):null}
        ...
        <View style={s.sectionWrap}>
          ...
          <View style={s.taskInputRow}>
            <TextInput
              value={taskText} onChangeText={setTaskText}
              placeholder="Nueva tarea..." placeholderTextColor="#55556a"
              style={s.taskInput} onSubmitEditing={handleAddTask} returnKeyType="done"
            />
            <TouchableOpacity onPress={handleAddTask} style={[s.taskAddBtn,{backgroundColor:accent}]} activeOpacity={0.8}>
          ...
          <View style={s.priorityRow}>
            {(['low','mid','high'] as Priority[]).map((p)=>(
              <TouchableOpacity
                key={p} onPress={()=>setTaskPriority(taskPriority===p?undefined:p)}
                style={[
                  s.priorityPickerBtn,
                  taskPriority===p && {borderColor:PRIORITY_COLORS[p],backgroundColor:PRIORITY_COLORS[p]+'18'},
                ]}
                activeOpacity={0.7}
              >
                <View style={[s.priorityDot,{backgroundColor:PRIORITY_COLORS[p]}]}/>
                <Text style={[s.priorityPickerTxt,taskPriority===p&&{color:PRIORITY_COLORS[p]}]}>
                  {PRIORITY_LABELS[p]}
                </Text>
              </TouchableOpacity>
            ))}
            {taskPriority && (
              <TouchableOpacity onPress={()=>setTaskPriority(undefined)} style={s.priorityClear}>
                <Text style={s.priorityClearTxt}>✕ Sin prioridad</Text>
              </TouchableOpacity>
            )}
          </View>
          {/* Task list */}
          {visibleTasks.length>0 ? (
            ...
            <View style={s.emptyTasks}><Text style={s.emptyTasksTxt}>Sin tareas pendientes ✓</Text></View>
          )}
        </View>
        ...
        <View style={s.sectionWrap}>
          <View style={s.habitsGrid}>
            {habits.map((h)=>{
              ...
              return (
                <HabitItem key={h.id} habit={h} done={done}
        ...
        <View style={[s.secRow,{marginTop:20}]}>
          ...
          <TouchableOpacity onPress={()=>router.push('/journal')}>
            <Text style={[s.secAction,{color:accent}]}>VER DIARIO →</Text>
          </TouchableOpacity>
        </View>
        <View style={s.sectionWrap}>
          <View style={s.moodRow}>
            {MOODS.map((m)=>(
              <TouchableOpacity
                ...
                style={[s.moodBtn,mood===m&&{borderColor:accent,backgroundColor:accent+'18'}]}
                activeOpacity={0.7}
              >
          ...
          <>
            <View style={[s.secRow,{marginTop:20}]}>
              <Text style={s.secLabel}>
                {template.name} · {template.days.map((d)=>dayNames[d]).join(' · ')}
              </Text>
              <TouchableOpacity onPress={()=>router.push('/(tabs)/schedule')}>
                <Text style={[s.secAction,{color:accent}]}>VER TODO →</Text>
              </TouchableOpacity>
            </View>
            <View style={s.sectionWrap}>
...
const s = StyleSheet.create({
  ...
  streakBadge:        {flexDirection:'row',alignItems:'center',gap:4,borderWidth:1,borderRadius:99,paddingHorizontal:12,paddingVertical:6},
  streakNum:          {fontFamily:MONO,fontSize:16,fontWeight:'700'},
  ...
  secAction:          {fontFamily:MONO,fontSize:9},
  ...
  taskAddBtn:         {width:42,height:42,borderRadius:12,alignItems:'center',justifyContent:'center'},
  ...
  priorityPickerBtn:  {flexDirection:'row',alignItems:'center',gap:5,paddingHorizontal:10,paddingVertical:6,borderRadius:99,borderWidth:1,borderColor:'#26262f',backgroundColor:'#141418'},
  priorityPickerTxt:  {fontFamily:MONO,fontSize:10,color:'#55556a'},
  priorityClear:      {paddingHorizontal:10,paddingVertical:6,borderRadius:99,backgroundColor:'#1c1c22'},
  priorityClearTxt:   {fontFamily:MONO,fontSize:10,color:'#55556a'},
  ...
  priorityBadge:      {flexDirection:'row',alignItems:'center',gap:4,paddingHorizontal:7,paddingVertical:3,borderRadius:99,borderWidth:1},
  priorityDot:        {width:5,height:5,borderRadius:2.5},
  priorityBadgeTxt:   {fontFamily:MONO,fontSize:9},
